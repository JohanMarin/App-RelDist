...
shinyServer(function(input, output){
  ...
  output$graficos <- renderPlot({
    ...
    ######################################################################
      ...
      curve(dWG(x, mu=input$mu.link, sigma=input$sigma.link, nu=input$nu.link ),
            ylab="Densidad", xlim=c(0,input$x), lwd=2, col="deepskyblue3",
            main="Función Densidad de Probabilidad", las = 1)
      grid()
      
      curve(pWG(mu=input$mu.link, sigma=input$sigma.link, nu=input$nu.link, x),
            xlab="x",ylab = "Confiabilidad", ylim=c(0,1), xlim=c(0,input$x),las=1,
            lwd=2, col="deepskyblue3", main= "Función de Confiabilidad")
      grid()
      
      curve(hWG(x, mu=input$mu.link, sigma=input$sigma.link, nu=input$nu.link ),
            ylab="Riesgo", xlim=c(0,input$x), lwd=2, las = 1, col="deepskyblue3",
            main= "Funcion de Riesgo")
      grid()
    ...
    ######################################################################
      ...
      curve(dKW(x, alpha=input$mu.link, theta=input$sigma.link, lambda=input$nu.link, tau=input$tau.link),
            ylab="Densidad", xlim=c(0,input$x), lwd=2, col="deepskyblue3",
            main="Función Densidad de Probabilidad", las = 1)
      grid()
  ...
  output$med.var <- renderText({
    ...
    ######################################################################
      ...
      fun1       <- function(x, mu, sigma) x * RelDists::dWG(x, mu = mu, 
                                                             sigma = sigma)
      esperanza  <- integrate (fun1, lower = 0, upper = Inf, mu = input$mu.link, 
                               sigma = input$sigma.link)
      fun2       <- function(x, mu, sigma) {
        media <- integrate (fun1, lower = 0, upper = Inf, mu = input$mu.link, 
                            sigma = input$sigma.link)$value
        (x-media)^2 * RelDists::dWG(x, mu = mu, sigma = sigma
        )}
      varianza   <- integrate (fun2, lower = 0, upper = Inf, mu = input$mu.link, 
                               sigma = input$sigma.link)
    ...
    ######################################################################
      ...
      varianza   <- integrate (fun2, lower = 0, upper = 1000, mu = input$mu.link, 
                               sigma = input$sigma.link)
    ...
    ######################################################################
      ...
      fun1       <- function(x, mu, sigma, nu, tau) x * RelDists::dGMW(y=x, mu = mu, 
                                                                 sigma = sigma,
                                                                 nu = nu, tau = tau)
      esperanza  <- integrate (fun1, lower = 0, upper = Inf, mu = input$mu.link, 
                               sigma = input$sigma.link, nu = input$nu.link,
      ...
      fun2       <- function(x, mu, sigma, nu, tau) {
        ...
        (x-media)^2 * RelDists::dGMW(mu = mu, sigma = sigma, nu = nu, tau = tau
        )}
      varianza   <- integrate (fun2, lower = 0, upper = Inf, mu = input$mu.link, 
                               sigma = input$sigma.link, nu = input$nu.link,
                               tau = input$tau.link)
    ...
    ######################################################################
      ...
      fun2       <- function(x, mu, sigma, nu, tau) {
        media <- integrate (fun1, lower = 0, upper = Inf, mu = input$mu.link, 
                            sigma = input$sigma.link, nu = input$nu.link,
                            tau = input$tau.link)$value
        (x-media)^2 * RelDists::dKW(mu = mu, sigma = sigma, nu = nu, tau = tau
        )}
      varianza   <- integrate (fun2, lower = 0, upper = Inf, mu = input$mu.link, 
                               sigma = input$sigma.link, nu = input$nu.link,
                               tau = input$tau.link)
